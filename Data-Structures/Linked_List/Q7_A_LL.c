...
void RecursiveReverse(ListNode **ptrHead)
{
	...
	/* 한국어: 여기에 코드를 작성하세요 */
	/*
		ptrHead는 head 변수의 주소
		*ptrHead는 head
		**ptrHead는 head가 가리키는 노드 내용
	*/

	ListNode *first, *rest;

	// 종료 조건: 리스트가 비었거나 노드가 1개뿐일 경우
	if (ptrHead == NULL || *ptrHead == NULL || (*ptrHead)->next == NULL)
    	return;

	first = *ptrHead; // 첫 노드
	rest = first->next; // 나머지 리스트의 시작
	
	// 나머지 리스트(rest)를 끝까지 재귀적으로 뒤집음
	RecursiveReverse(&rest);

	first->next->next = first; // first의 다음 노드가 first를 가리키도록 하여 링크 방향을 반대로 바꿈
    first->next = NULL; // 기존의 정방향 링크를 끊어 순환이 생기지 않도록 함
    *ptrHead = rest; // 뒤집힌 나머지 리스트의 시작 노드를 새 head로 갱신함
